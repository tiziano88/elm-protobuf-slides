...
Elm.Main.make = function (_elm) {
   ...
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([A2(_op["=>"],"padding","1em")
                                              ,A2(_op["=>"],"margin","auto")
                                              ,A2(_op["=>"],"width","40em")
                                              ,A2(_op["=>"],"font-family","\'Ubuntu\'")
                                              ,A2(_op["=>"],
                                              "font-size",
                                              function () {
                                                 var _p0 = model.size;
                                                 var x = _p0._0;
                                                 var y = _p0._1;
                                                 return _U.cmp($Basics.toFloat(x) / $Basics.toFloat(y),ratio) < 0 ? "2vw" : "3vh";
                                              }())]))]),
      _U.list([A3($Html.node,"script",_U.list([$Html$Attributes.src("./highlight/highlight.pack.js")]),_U.list([]))
              ,A3($Html.node,
              "link",
              _U.list([$Html$Attributes.rel("stylesheet"),$Html$Attributes.href("http://fonts.googleapis.com/css?family=Ubuntu")]),
              _U.list([]))
              ...
              _U.list([$Html$Attributes.style(_U.list([A2(_op["=>"],"background-color","white")
                                                      ,A2(_op["=>"],"box-shadow","0 0 10px")
                                                      ,A2(_op["=>"],"border-radius","10px")
                                                      ,A2(_op["=>"],"padding","2em 3em")]))]),
              _U.list([A2($Html.div,
                      _U.list([$Html$Attributes.style(_U.list([A2(_op["=>"],"height","25em"),A2(_op["=>"],"overflow","auto")]))]),
   ...
   var SetCurrentPage = function (a) {    return {ctor: "SetCurrentPage",_0: a};};
   var SetContent = function (a) {    return {ctor: "SetContent",_0: a};};
   var Nop = {ctor: "Nop"};
   var updateHash = function (m) {
      return {ctor: "_Tuple2"
             ,_0: m
             ...
             $History.replacePath(A2($Basics._op["++"],"#",$Basics.toString(m.currentPage + 1)))))};
   };
   var update = F2(function (action,model) {
      ...
      {case "Nop": return noEffects(model);
         ...
         case "SetCurrentPage": return noEffects(_U.update(model,{currentPage: _p1._0}));
         case "NextPage": var newPage = model.currentPage + 1;
           return _U.cmp(newPage,$Array.length(model.pages)) < 0 ? updateHash(_U.update(model,{currentPage: newPage})) : noEffects(model);
         case "PreviousPage": var newPage = model.currentPage - 1;
           return _U.cmp(newPage,0) > -1 ? updateHash(_U.update(model,{currentPage: newPage})) : noEffects(model);
         default: return noEffects(_U.update(model,{size: _p1._0}));}
   });
   var initialModel = {content: "",pages: $Array.empty,currentPage: 0,size: {ctor: "_Tuple2",_0: 0,_1: 0}};
   var $const = F2(function (x,_p2) {    return x;});
   var getLocation = $Effects.task(A2($Task.map,
   SetCurrentPage,
   A2($Task.map,
   function (x) {
      return x - 1;
   ...
   A2($Task.map,$String.toInt,A2($Task.map,$String.dropLeft(1),A2($Task.map,function (l) {    return l.hash;},$Location.location)))))));
   var url = "./slides.md";
   var getContent = $Effects.task(A2($Task.map,SetContent,A2($Task.map,$Maybe.withDefault(""),$Task.toMaybe($Http.getString(url)))));
   var init = {ctor: "_Tuple2",_0: initialModel,_1: $Effects.batch(_U.list([getContent,getLocation]))};
   var app = $StartApp.start({init: init
                             ,view: view
                             ,update: update
                             ,inputs: _U.list([A2($Signal.map,Resize,$Window.dimensions)
                                              ,A2($Signal.map,function (s) {    return s ? NextPage : Nop;},$Keyboard.space)
                                              ...
                                              $Keyboard.arrows)])});
   var Model = F4(function (a,b,c,d) {    return {content: a,pages: b,currentPage: c,size: d};});
