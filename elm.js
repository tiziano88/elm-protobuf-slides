...
Elm.Main.make = function (_elm) {
   ...
   var update = F2(function (action,model) {
      ...
      {case "Nop": return noEffects(model);
         ...
         case "NextPage": return noEffects(_U.update(model,{currentPage: model.currentPage + 1}));
         default: return noEffects(_U.update(model,{currentPage: model.currentPage - 1}));}
   });
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A3($Html.node,"script",_U.list([$Html$Attributes.src("/highlight/highlight.pack.js")]),_U.list([]))
              ...
              _U.list([A2($Html.div,
                      ...
                      ,A2($Html.div,
                      _U.list([]),
                      _U.list([A2($Html.a,_U.list([A2($Html$Events.onClick,address,PreviousPage)]),_U.list([$Html.text("<<<")]))
                              ,$Html.text($Basics.toString(model.currentPage))
                              ,A2($Html.a,_U.list([A2($Html$Events.onClick,address,NextPage)]),_U.list([$Html.text(">>>")]))]))]))]));
   });
